// ========== MÓDULO DE COMPARTIR CON IMAGEN (CORREGIDO) ==========
class ShareManager {
  constructor() {
    this.enlaceGenerado = '';
        ...
        <div class="share-options">
          ...
          <div class="share-option">
            ...
            <label for="shareImage">
              ...
              <small>Envía la imagen del código QR (si la plataforma lo permite)</small>
            </label>
          </div>
  ...
  // Obtener blob del QR usando API correcta (.getBlob())
  async generateQRBlob() {
    ...
    try {
      const blob = await this.qrInstance.getBlob();
      if (!blob) throw new Error('getBlob() devolvió null o undefined');
      this.qrBlob = blob;
      if (this.qrImageUrl) {
  ...
  generateShareMessage(includeLink = true) {
    ...
    message += `Escanea el código QR para registrar la cotización.`;
    
    return message;
  }
  
  // Compartir por WhatsApp (comportamiento por plataforma)
  async shareWhatsApp() {
    try {
      let shareType = this.getShareType();
      const message = this.generateShareMessage(shareType !== 'image');

      // Si el usuario eligió imagen o ambos y estamos en Android intentamos el truco desde mobile-share
      const isAndroid = /Android/i.test(navigator.userAgent);

      if ((shareType === 'image' || shareType === 'both') && isAndroid && window.mobileShareManager) {
        // Pedimos a MobileShareManager el flujo especial para Android
        try {
          await window.mobileShareManager.shareToWhatsAppAndroidWithImage();
          return;
        } catch (err) {
          console.warn('Fallo intento de enviar imagen por WhatsApp Android:', err);
          // si falla, caemos a compartir sólo texto
        }
      }

      // Por defecto: abrir wa.me con texto
  ...
  // Compartir por correo (con la guía para adjuntar)
  async shareEmail() {
    try {
      ...
      if (shareType === 'image' || shareType === 'both') {
        try {
          ...
          this.showNotification('Se generó la imagen. Descárgala y adjúntala al correo si tu cliente no permite adjuntos desde web.', 'info');
        } catch (err) {
          // ignore and continue with mailto
        }
      }
  ...
  async shareTelegram() {
    try {
      ...
      // Telegram web no permite adjuntar archivos desde JS; usamos URL de share
  ...
  async shareSMS() {
    try {
      let shareType = this.getShareType();
      if (shareType === 'image' || shareType === 'both') {
        this.showNotification('SMS no soporta imágenes. Se enviará enlace/texto.', 'info');
        shareType = 'link';
      }
  ...
  overrideGlobalFunctions() {
    ...
    window.generarQR = () => {
      ...
      if (window.shareManager) {
        ...
        // actualizar instancia qr si existe
        setTimeout(() => {
          if (window.qr) {
